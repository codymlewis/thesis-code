"""
Network library for federated learning computations, mainly the TTP
...
class Network:
    ...
    def setup(self):
        "Initialize and collect the keys for the secure aggregation"
    ...
    def begin(self, subject="grads"):
        "Prepare the clients for their computation"
        for c in self.clients:
            self.params_len = c.prepare_for_agg(subject)

    def __call__(self, R):
        "Run a step of the secure aggregation algorithm"
    ...
    def send_grads(self, grads):
        "Send the global model update to the clients"
        for c in self.clients:
